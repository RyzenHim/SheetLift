...
const Saved = () => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const { data } = await axios.get(
        "https://sheetlift-production.up.railway.app/api/tasks",
        {
          ...authHeader(),
  ...
  const handleAdd = async (e) => {
    ...
    try {
      await axios.post(
        "sheetlift-production.up.railway.app/api/tasks",
        addForm,
        authHeader(),
      );
  ...
  const saveEdit = async (taskId) => {
    try {
      await axios.put(
        `sheetlift-production.up.railway.app/api/tasks/${taskId}`,
        editForm,
        authHeader(),
      );
      setEditingId(null);
  ...
  const toggleDone = async (task) => {
    try {
      await axios.put(
        `sheetlift-production.up.railway.app/api/tasks/${task._id}`,
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            {!search && (
              <a
                href="/landing"
                className="inline-block px-8 py-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold rounded-2xl hover:from-blue-600 hover:to-indigo-700 transition-all shadow-lg"
              >
                Go Load from Sheets
              </a>
            )}
              ...
              <table className="w-full table-auto text-sm">
                <thead>
                  <tr className="bg-gradient-to-r from-purple-500 to-pink-500 text-white">
                    <th className="px-5 py-3 text-left rounded-tl-xl font-bold">
                      #
                    </th>
                    ...
                    <th className="px-5 py-3 text-left rounded-tr-xl font-bold">
                      Actions
                    </th>
                  </tr>
                </thead>
                      ...
                      >
                        ...
                        {isEditing ? (
                          <>
                            ...
                            <td className="px-5 py-3">
                              <button
                                ...
                                className="mr-1 px-3 py-1 bg-green-500 text-white rounded-lg hover:bg-green-600 text-xs font-semibold"
                              >
                                Save
                              </button>
                              <button
                          ...
                          <>
                            ...
                            <td className="px-5 py-3">
                              <span
                                className={`px-3 py-1 rounded-full text-xs font-bold ${task.completed ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                              >
                            ...
                            <td className="px-5 py-3">
                              <button
                                ...
                                className="mr-1 px-3 py-1 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 text-xs font-semibold"
                              >
                                {task.completed ? "Undo" : "✓ Done"}
                              </button>
                              <button
                                onClick={() => startEdit(task)}
                                className="mr-1 px-3 py-1 bg-yellow-400 text-white rounded-lg hover:bg-yellow-500 text-xs font-semibold"
                              >
                                Edit
                              </button>
                              <button
