...
const Landing = () => {
  ...
  const loadSheet = () => {
    ...
    Papa.parse(csvUrl, {
      ...
      complete: ({ data }) => {
        ...
        if (tasks.length === 0) {
          setError(
            "No valid rows found. Make sure your sheet has title, description, dueDate columns.",
          );
        } else {
          setCsvTasks(tasks);
          setAllTasks(tasks);
  ...
  const saveAllToDb = async () => {
    ...
    try {
      await axios.post(
        "https://sheetlift-production.up.railway.app/api/tasks/bulk-create",
        { tasks: allTasks },
        authHeader(),
      );
      setCsvTasks([]);
      ...
      setError(
        err.response?.data?.message ||
          "Save failed. Make sure you are logged in.",
      );
    }
    setSavingAll(false);
  };
  ...
  const addTask = (e) => {
    ...
    setAllTasks([
      { tempId: Date.now() + Math.random(), ...addForm, completed: false },
      ...allTasks,
    ]);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white shadow-xl rounded-3xl p-8 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">
            Publish your sheet as CSV (File → Share → Publish to web → CSV) and
              ...
              <table className="w-full table-auto text-sm">
                <thead>
                  <tr className="bg-gradient-to-r from-blue-500 to-purple-500 text-white">
                    ...
                    <th className="px-5 py-3 text-left rounded-tr-xl">
                      Actions
                    </th>
                  </tr>
                </thead>
                      ...
                      >
                        {isEditing ? (
                          ...
                          <>
                            ...
                            <td className="px-5 py-3">
                              <span
                                className={`px-2 py-1 rounded-full text-xs font-bold ${task.completed ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                              >
